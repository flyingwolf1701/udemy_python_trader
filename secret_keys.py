...
print(
    f"[DEBUG from secret_keys.py] Loaded CRYPTO_API_KEY: '{Secrets.CRYPTO_API_KEY}' (Type: {type(Secrets.CRYPTO_API_KEY)})"
)
print(
    f"[DEBUG from secret_keys.py] Loaded CRYPTO_API_SECRET: '{Secrets.CRYPTO_API_SECRET[:5] if Secrets.CRYPTO_API_SECRET else None}...' (Type: {type(Secrets.CRYPTO_API_SECRET)})"
)
