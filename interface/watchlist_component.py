...
class Watchlist(tk.Frame):
    def __init__(
        ...
        CryptoExchange_contracts: typing.Dict[str, Contract],
        *args,
        **kwargs,
    ):
        ...
        self.CryptoExchange_symbols = list(CryptoExchange_contracts.keys())
        ...
        self._CryptoExchange_label = tk.Label(
            self._commands_frame,
            text="crypto",
            bg=BG_COLOR,
            fg=FG_COLOR,
        ...
        self._CryptoExchange_label.grid(row=0, column=1)

        self._CryptoExchange_entry = tk.Entry(
            self._commands_frame,
            fg=FG_COLOR,
            justify=tk.CENTER,
            insertbackground=FG_COLOR,
        ...
        self._CryptoExchange_entry.bind("<Return>", self._add_CryptoExchange_symbol)
        self._CryptoExchange_entry.grid(row=1, column=1)
    ...
    def _add_CryptoExchange_symbol(self, event):
        symbol = event.widget.get()

        if symbol in self.CryptoExchange_symbols:
    ...
    def _add_symbol(self, symbol: str, exchange: str):
        ...
        self.body_widgets["symbol"][b_index] = tk.Label(
            self._table_frame, text=symbol, bg=BG_COLOR, fg=FG_COLOR_2, font=GLOBAL_FONT
        )
